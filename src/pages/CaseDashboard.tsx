...
function CaseRow({ caseData, onOpen }: { caseData: CaseItem; onOpen: (c: CaseItem) => void }) {
  ...
  return (
    ...
    >
      <div style={COL.id}>
        <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, letterSpacing: '0.03em', color: isHighlit ? '#5FA8D3' : '#9494A0', transition: 'color 80ms' }}>
          {caseData.id}
        </span>
      </div>
      ...
      <div style={COL.anomalies}>
        <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: caseData.anomalyCount > 0 ? '#F59E0B' : '#4B5563' }}>
      ...
      <div style={COL.action}>
        <span style={{ fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 500, transition: 'color 80ms', color: isHighlit ? '#5FA8D3' : '#38383F', fontFamily: 'Inter, sans-serif' }}>
          Open →
        </span>
      </div>
    </div>
