      ...
      <div className={`relative z-10 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] min-h-[100dvh] flex flex-col ${drawerOpen ? 'brightness-50' : 'brightness-100'}`}>
        ...
        <nav className="fixed top-0 left-0 right-0 h-16 flex items-center justify-between px-6 md:px-8 backdrop-blur-md bg-black/40 border-b border-white/10 z-20">
          <div 
            ...
            className="text-white font-bold text-lg tracking-[0.2em] uppercase cursor-pointer hover:text-neon-cyan transition-colors"
          >
            Phishield
          </div>
          <div className="flex items-center gap-4">
            <button 
              ...
              className="text-white p-2 border border-white/10 hover:bg-white/5 transition-colors rounded-none"
              title="Search System"
            >
            ...
            <button 
              onClick={() => navigate('/dashboard')}
              className="text-white p-2 border border-white/10 hover:bg-white/5 transition-colors rounded-none mr-2"
              title="Open Phishield Platform"
            >
          ...
          <div className="w-full max-w-6xl mx-auto flex flex-col items-center">
            ...
            >
              <button
                ...
                className="bg-neon-cyan text-black px-8 py-3 text-sm font-semibold tracking-widest uppercase hover:bg-cyan-300 transition-colors shadow-[0_0_20px_rgba(0,240,255,0.4)]"
              >
                Launch Platform →
              </button>
              <button
                onClick={() => setDrawerOpen(true)}
                className="border border-white/20 text-white px-8 py-3 text-sm font-semibold tracking-widest uppercase hover:bg-white/10 transition-colors"
              >
                Request Access
              </button>
            </motion.div>
            ...
            {formFields.map((field, i) => (
              ...
              >
                ...
                <input 
                  ...
                  className="w-full bg-transparent border-b border-gray-300 focus:border-black focus:outline-none py-1.5 text-black text-sm transition-colors rounded-none"
                />
              </motion.div>
            ))}
